import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/Bill/BillsCell'
import { timeTag, truncate } from 'src/lib/formatters'

import type { DeleteBillMutationVariables, FindBills } from 'types/graphql'

const DELETE_BILL_MUTATION = gql`
  mutation DeleteBillMutation($id: Int!) {
    deleteBill(id: $id) {
      id
    }
  }
`

const BillsList = ({ bills }: FindBills) => {
  const [deleteBill] = useMutation(DELETE_BILL_MUTATION, {
    onCompleted: () => {
      toast.success('Bill deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id: DeleteBillMutationVariables['id']) => {
    if (confirm('Are you sure you want to delete bill ' + id + '?')) {
      deleteBill({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Total</th>
            <th>Created at</th>
            <th>Updated at</th>
            <th>Customer profile id</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {bills.map((bill) => (
            <tr key={bill.id}>
              <td>{truncate(bill.id)}</td>
              <td>{truncate(bill.total)}</td>
              <td>{timeTag(bill.createdAt)}</td>
              <td>{timeTag(bill.updatedAt)}</td>
              <td>{truncate(bill.customerProfileId)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.bill({ id: bill.id })}
                    title={'Show bill ' + bill.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.editBill({ id: bill.id })}
                    title={'Edit bill ' + bill.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete bill ' + bill.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(bill.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default BillsList
